...
qnet:add(nn.Bootstrap(nn.Linear(hiddenSize,actionRange), headNum, param_init))
--print(qnet:forward(torch.rand(4,stateDim))) -- test
...
local bdqn_param = {headNum = headNum, replaySize = 256, batchSize = 16, discount = 0.99, epsilon = 0.05}
local bdqn = dprl.bdqn(qnet, bdqn_param, optimMethod, optimConfig)
-- initialize dql
local bdql_param = {step = 256, lr = 0.01, updatePeriod = 64}
...
local bdql = dprl.bdql(bdqn, env, bdql_param, preprop, actPreprop)
